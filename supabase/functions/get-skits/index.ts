...
serve(async (req) => {
  ...
  try {
    console.log('Starting get-skits function execution')
    ...
    try {
      ...
      const baseUrl = 'https://www.googleapis.com/youtube/v3/search?part=snippet&q=nollywood+skit+comedy&type=video&order=date&maxResults=50';
    ...
    console.error('Fatal error in get-skits function:', error);
    return new Response(JSON.stringify([]), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  }
