...
export class ParticleSystem {
  ...
  public updateParticles(t: number, textParticles: TextParticle[]) {
    if (this.isSecondAnimation) {
      ...
      const cycleT = t % 1; // Create a continuous cycle
      for (let i = 0; i < this.particleCount; i++) {
        ...
        const wobble = Math.sin(cycleT * Math.PI * 2 + i * 0.1) * 0.05;
        this.positions[i3] += (targetX + wobble - this.positions[i3]) * 0.08;
        this.positions[i3 + 1] += (targetY + wobble - this.positions[i3 + 1]) * 0.08;
        this.positions[i3 + 2] += (wobble - this.positions[i3 + 2]) * 0.08;
      }
    } else {
      // First animation: original particle movement and text formation
      for (let i = 0; i < this.particleCount; i++) {
        const i3 = i * 3;
        const targetIndex = i % textParticles.length;
        ...
        if (t < 0.3) {
          ...
          const radius = 4 + Math.sin(t * Math.PI * 6) * 1.5;
